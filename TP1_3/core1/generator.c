...
void ShutdownTaskGenerateFct(OS_TMR *p_tmr, void *p_arg)
{
    ...
    // Arr�ter les t�ches et lib�rer les ressources
    delete_tasks();
}
...
void TaskGenerate(void *data) {
	...
	volatile uint32_t* req = (uint32_t*) (BASEADDR + 0x00);	// signal comme quoi on est pr�t � recevoir un data
...
//			xil_printf("GENERATE: Debut rafale no: %d  avec %d paquets\n", burst_number, *number_of_packets);
...
//			xil_printf("GENERATE: Fin rafale no: %d  avec %d paquets\n", burst_number, *number_of_packets);
