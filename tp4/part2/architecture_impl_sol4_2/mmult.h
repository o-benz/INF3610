...
#define MCR_SIZE 40000				  
typedef ap_axiu<16,4,5,5> AXI_VAL;
...
template <typename T, int U, int TI, int TD> T pop_stream(ap_axiu <sizeof(T)*8,U,TI,TD> const &e)
{
#pragma HLS INLINE
...
int test_matrix_mult(void)
{
	...
	for(i = 0; i<DIM; i++)
		for(j = 0; j<DIM; j++)
			matOp1[i][j] = (short)(i+j);

	for(i = 0; i<DIM; i++)
		for(j = 0; j<DIM; j++)
			matOp2[i][j] = (short)(i*j);
	/** End of Initiation */
